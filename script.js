...
const render = (q) => {
  ...
  let html = `${current + 1}. ${q.question}`;
  if (q.image)
    html += `<br><img src="${q.image}" style="max-width:100%;border-radius:8px">`;
  html += `<div class="options">`;
  q.options.forEach((t, i) => {
    const v = String.fromCharCode(65 + i);
    html += `<label><input type="${type}" name="q" value="${v}">${v}. ${t}</label>`;
  });
  html += `</div>`;
...
const submitQuiz = () => {
  ...
  const answers =
    mode === "batch"
      ...
      : [
          [...document.querySelectorAll('input[name="q"]:checked')].map((e) =>
            e.value
          ).sort(),
        ];
  selected.forEach((q, i) => {
    ...
    else wrong.push({ ...q, user: user.join("") });
  });
  const res = document.getElementById("result");
  res.innerHTML =
    `<h2>得分：${score}/${selected.length}</h2>` +
    `<p style="color:${
      score >= pass ? "var(--success)" : "var(--danger)"
    }">${score >= pass ? "恭喜通过！" : "未通过，请继续努力！"}</p>`;
  if (wrong.length) {
    res.innerHTML += "<h3>错题回顾：</h3>";
    wrong.forEach(
      (w) =>
        (res.innerHTML += `<div><p><strong>题目：</strong>${w.question}</p><p><strong>你的答案：</strong><span style="color:var(--danger)">${w.user}</span></p><p><strong>正确答案：</strong><span style="color:var(--success)">${w.correct}</span></p></div>`)
    );
  }
  ["quiz", "timer", "submitBtn", "nextBtn"].forEach(
...
const startQuiz = async () => {
  ...
  selected = pick(l);
  timeLeft = { A: 2400, B: 3600, C: 5400 }[l]; // 秒
  document.getElementById("quiz").innerHTML = selected
    .map((q, i) => {
      ...
      q.options.forEach((t, j) => {
        ...
        html += `<label><input type="${type}" name="q${i}" value="${v}">${v}. ${t}</label>`;
      });
      html += `</div></div>`;
      return html;
    })
